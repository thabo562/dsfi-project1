...
By thoughtfully selecting and configuring these hyperparameters, the study aims to optimise the FFNN's predictive performance, achieving a balance between model precision, training efficiency, and resistance to overfitting. @fig-NN-Acc-BoW shows the accuracy and loss of the FFNN using bag of words as the number of epochs increases.
...
The test dataset yielded an accuracy of 45.9%, which is lower than the accuracy achieved by the previous BoW model. This suggests that the FFNN employing BoW as input features may not perform as effectively as a Random Forest classifier when utilizing BoW representations. @fig-NN-Acc-TF shows the accuracy and loss of the FFNN using TF-IDF as the number of epochs increases.
